...
def train():
    ...
    for step in range(1, args.max_iter + 1):
        ...
        src_feat = F.adaptive_avg_pool2d(source_model.get_conv_feat(src_im_data), 1).view(1, 512)
        if args.tanh:
        ...
        tar_feat = F.adaptive_avg_pool2d(target_model.get_conv_feat(tar_im_data), 1).view(1, 512)
        if args.tanh:

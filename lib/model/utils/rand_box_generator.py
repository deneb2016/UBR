...
class NaturalBoxGenerator:
    def __init__(self, iou_th, seed_pool_size=100000):
        ...
        while cnt < seed_pool_size:
            ...
            dx = torch.FloatTensor(np.random.uniform(-pos_th, pos_th, seed_pool_size * 10))
            dy = torch.FloatTensor(np.random.uniform(-pos_th, pos_th, seed_pool_size * 10))
            dw = torch.FloatTensor(np.random.uniform(scale_th_min, scale_th_max, seed_pool_size * 10))
            dh = torch.FloatTensor(np.random.uniform(scale_th_min, scale_th_max, seed_pool_size * 10))
...
class NaturalUniformBoxGenerator:
    def __init__(self, iou_th, seed_pool_size=100000, pos_th=0.35, scale_min=0.5, scale_max=1.5):
        self._seed_pool_size = seed_pool_size
        self._delta = torch.zeros((seed_pool_size, 4))
        self._iou_th = iou_th
        ...
        while cnt < seed_pool_size:
            ...
            dx = torch.FloatTensor(np.random.uniform(-pos_th, pos_th, seed_pool_size * 10))
            dy = torch.FloatTensor(np.random.uniform(-pos_th, pos_th, seed_pool_size * 10))
            dw = torch.FloatTensor(np.exp(np.random.uniform(scale_th_min, scale_th_max, seed_pool_size * 10)))
            dh = torch.FloatTensor(np.exp(np.random.uniform(scale_th_min, scale_th_max, seed_pool_size * 10)))
